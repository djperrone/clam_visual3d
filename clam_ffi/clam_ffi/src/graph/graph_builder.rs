...
use crate::{
    ffi_impl::cluster_ids::ClusterID, graph, h_graph, handle::handle::Handle, utils::{
        error::FFIError,
...
//             let spring = Spring::new(c1.distance_to_other(data, c2), ClusterID::from_cluster(&c1), ClusterID::from_cluster(&c2));
//             edges.push(spring);
//         }
//     }
// }
...
pub fn build_force_directed_graph_async(
    ...
    scalar: f32,
    max_iters: i32,
) -> Result<(JoinHandle<()>, Arc<ForceDirectedGraphAsync>), FFIError> {
    if let Some(tree) = handle.tree() {
        if let Some(clam_graph) = handle.clam_graph() {

            // let g = Arc::new(h_graph::h_graph::ForceDirectedGraph::new(
            //     tree, clam_graph, scalar, max_iters as usize,
            // ));

           if let Ok(fdg) =  h_graph::h_graph::ForceDirectedGraph::new(
            tree, clam_graph, scalar, max_iters as usize,){

            
            let g = Arc::new(ForceDirectedGraphAsync::new( fdg, scalar, max_iters, clam_graph.max_depth()));
...
//     }
    ...
    // ForceDirectedGraphAsync::new(graph, springs, scalar, max_iters)
// }
