...
use abd_clam::{Cluster, Dataset, PartitionCriteria, Tree};
use glam::Vec3;
...
use crate::{
    ...
    utils::{
        ...
        types::{DataSetf32, Treef32, Vertexf32},
    },
};

use super::{
    false_nearest_neighbors::{find_all_knn_original, knn_naive_original, FNN_Wrapper},
    utils,
};
                    ...
                    if let Ok(entry) = entry {
                        ...
                        if entry_path.is_dir() {
                            ...
                            if let Some(positions_file) = entry_path.to_str() {
                                if let Ok(positions) = utils::read_umap_positions(positions_file) {
                                    println!("positions file : {}", positions_file);
                                    let graph_k = utils::extract_umap_k(positions_file).unwrap();
                                    for test_k in 3..20 {
                                        let (precision, recall, f1_score) =
                                            umap_test_fnn_score(&positions, &tree, test_k).unwrap();
                                        let mut file_path = PathBuf::new();
                                        // file_path.push("triangle_acc_results");
                                        file_path.push(out_folder);
                                        file_path.push(tree.data().name());
                                        file_path.push(
                                            "graph-k_".to_string() + graph_k.to_string().as_str(),
                                        );

                                        file_path
                                            .push("k_".to_string() + test_k.to_string().as_str());
