...
use crate::{ffi_impl::cluster_ids::ClusterID, utils::{
    error::FFIError,
...
impl ForceDirectedGraph {
    ...
    fn build(
        ...
        graph.insert((left_cluster.offset(), left_cluster.cardinality()), left_node);
        graph.insert((right_cluster.offset(), right_cluster.cardinality()), right_node);
    ...
    fn split_nodes(
        ...
        let mut next_graph: HashMap<(usize, usize), PhysicsNode> = HashMap::new();
        ...
        for (id, node) in &self.graph {
            ...
            if clam_graph.ordered_clusters().contains(&cluster) {
                next_graph.insert((cluster.offset(), cluster.cardinality()), *node);
            } else {
                ...
                next_graph.insert(left_id.to_tuple(), left_node);
                next_graph.insert(right_id.to_tuple(), right_node);
    ...
    fn split_springs(
        ...
        for spring in &self.springs {
            ...
            let (left_id, right_id) = spring.get_node_ids();
            if self.graph.contains_key(&left_id.to_tuple()) && self.graph.contains_key(&right_id.to_tuple()) {
            ...
            if self.graph.contains_key(&left_id.to_tuple()) {
                let cluster = utils::get_cluster(tree, &left_id.to_tuple())?;
            ...
            if self.graph.contains_key(&right_id.to_tuple()) {
                let cluster = get_cluster(tree, &right_id.to_tuple())?;
    ...
    fn cleanup_helper(
        ...
        if let Some(relation_threshold) = relation_threshold {
            if spring.relation() > relation_threshold {
                ...
                let c2 = get_cluster(tree, &ids.1.to_tuple())?;
    ...
    pub fn split_node<'a>(
            ...
            utils::calc_triangle_distances(cluster, left, right, tree, scalar, max_edge_len)
        {
            ...
            return Ok((
                ...
                Spring::new(
                    dist_between_children,
                    ClusterID::from_cluster(left),
                    ClusterID::from_cluster(right),
                    0,
                ),
            ));
        } else {
    ...
    pub fn get_cluster_position(&self, id: &(usize, usize)) -> Result<Vec3, String> {
    ...
    pub fn contains(&self, id: &(usize, usize)) -> bool {
...
fn assert_graphs_equivalent(clam_graph: &Graphf32, fdg: &ForceDirectedGraph, tree: &Treef32) {
    ...
    for c in clam_graph.ordered_clusters() {
        assert!(fdg.contains(&(c.offset(), c.cardinality())));
    }
    ...
    for edge in clam_graph.edges() {
        ...
        assert!(fdg_contains_spring(fdg, &ClusterID::from_cluster(left), &ClusterID::from_cluster(right)));
    }
}

fn fdg_contains_spring(fdg: &ForceDirectedGraph, left: &ClusterID, right: &ClusterID) -> bool {
